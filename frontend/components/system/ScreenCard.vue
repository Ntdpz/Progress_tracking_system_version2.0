<template>
  <div>
    ...
    <v-dialog v-model="addScreenDialog" max-width="800px">
      <add-form :users="users" :systemId="system.id" :projectId="system.project_id" @closeDialog="handleCloseDialog">
      </add-form>
    </v-dialog>
  </div>
</template>
...
export default {
  ...
  async asyncData({ params }) {
    const system_id = params.id;
    try {
      // Fetch system details
      const systemResponse = await axios.get(`http://localhost:7777/systems/getOne/${system_id}`);
      ...
      // Add project_id and system_id to filtered users
      ...
      return { system, screens, users: filteredUsers, user_systems, projectId, systemId };
    } catch (error) {
