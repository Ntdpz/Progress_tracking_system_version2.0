...
router.post("/createUser_project", async (req, res) => {
  ...
  try {
    for (let i = 0; i < user_ids.length; i++) {
      // เปลี่ยนชื่อตัวแปรจาก user_id เป็น user_ids
      ...
      connection.query(
        ...
        (err, results, fields) => {
          if (err) {
            console.log(
              "Error while checking for duplicate user_projects in the database",
              err
            );
            return;
          }
          if (results.length > 0) {
            ...
            connection.query(
              ...
              (err, results, fields) => {
                if (err) {
                  console.log(
                    "Error while inserting a user_project into the database",
                    err
                  );
                  return;
                }
...
router.delete(
  "/deleteUserProjectById/:project_id/:user_id",
  async (req, res) => {
