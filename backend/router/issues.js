...
router.post("/createIssue", async (req, res) => {
  ...
  try {
    connection.query(
      "INSERT INTO issues (screen_id, system_id, project_id, issue_name, issue_id, issue_type, issue_informer, issue_priority, issue_end, issue_assign, issue_qc, issue_des, issue_des_sa, issue_doc_id, issue_customer, issue_filename, issue_des_dev, issue_des_implementer, issue_start, issue_expected, issue_status, issue_accepting, issue_manday, issue_complete) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        screen_id,
        system_id,
        project_id,
  ...
  try {
    connection.query(
      ...
      (err, results, fields) => {
        ...
        return res
          .status(200)
          .json({ message: "System issue successfully!" });
      }
    );
  } catch (err) {
    console.log(err);
